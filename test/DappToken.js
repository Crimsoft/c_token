...
contract("DappToken", function(accounts) {
	...
	it("approves tokens for delegated transfers", function() {
		return DappToken.deployed().then(function(instance) {
			...
			assert.equal(receipt.logs[0].args._owner, accounts[0], "logs the authorizer's/owner's account");
			assert.equal(receipt.logs[0].args._spender, accounts[1], "logs the spender's/authorized account");
	...
	it("handles delegated transfers", function() {
		return DappToken.deployed().then(function(instance) {
			...
			// try transferring a value larger that the sender's balance
			return tokenInstance.transferFrom(fromAccount, toAccount, 9999, { from: spendingAccount });
		}).then(assert.fail).catch(function(error) {
			assert(error.message.indexOf("revert") >= 0, "insufficient balance");

			// try transferring a value larger than the approved amount
			return tokenInstance.transferFrom(fromAccount, toAccount, 20, { from: spendingAccount });
		}).then(assert.fail).catch(function(error) {
			assert(error.message.indexOf("revert") >= 0, "transfer amount is more than the approved amount");
			...
			assert.equal(receipt.logs.length, 1, "triggers one event");
			assert.equal(receipt.logs[0].event, "Transfer", "should be the 'Transfer' event");
			assert.equal(receipt.logs[0].args._from, fromAccount, "logs the authorized/sender's account");
			assert.equal(receipt.logs[0].args._to, toAccount, "logs the receiving account");
			assert.equal(receipt.logs[0].args._value, 10, "logs the transfer amount");
			return tokenInstance.balanceOf(fromAccount);
		}).then(function(balance) {
			assert.equal(balance.toNumber(), 90, "deducts the amount from the sender's account");
